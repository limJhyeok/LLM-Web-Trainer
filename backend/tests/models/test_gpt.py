...
def test_overfit_a_batch(gpt, device, tiny_shakespeare):
    ...
    if len(tokens) < BATCH_SIZE * SEQUENCE_LENGTH + 1:
        raise ValueError(f"Insufficient data: required {BATCH_SIZE * SEQUENCE_LENGTH + 1}, but got {len(tokens)}")
    ...
    for i in range(NUM_EPOCHS):
        ...
        if loss.item() <= LOSS_THRESHOLD:
            print(f"Training stopped early at step {i + 1}. Loss reached {loss.item():.4f}")
            break

    # Assert final loss
    assert loss <= LOSS_THRESHOLD, f"Final loss {loss.item()} did not meet the threshold {LOSS_THRESHOLD}"